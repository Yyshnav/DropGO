...
// Future<void> seekAudio(String messageId, Duration position) async {
//   final player = audioPlayers[messageId];
//   if (player != null && player.isOpen()) {
//     try {
//       // Ensure position is within valid range
//       final duration = durationMap[messageId]?.value ?? Duration.zero;
//       final clampedPosition = Duration(
//         milliseconds: position.inMilliseconds.clamp(0, duration.inMilliseconds),
//       );
//       await player.seekToPlayer(clampedPosition);
//       positionMap[messageId]?.value = clampedPosition; // Update position immediately
//       print('Seeked to: $clampedPosition for messageId: $messageId');
//       update(); // Notify GetX UI
//     } catch (e) {
//       print('Error seeking audio: $e');
//       Get.snackbar('Error', 'Failed to seek audio: $e');
//     }
//   } else {
//     print('Player not found or not open for messageId: $messageId');
//     Get.snackbar('Error', 'Audio player not available');
//   }
// }
...
class ChatController extends GetxController {
  ...
  // CORRECTED: Added declaration for _positionSub
  StreamSubscription? _positionSub;
  ...
  // CORRECTED: Added method to initialize player
  ...
  Future<void> _loadHistory() async {
    ...
    try {
      ...
      if (msg.mediaType == MediaType.audio) {
        initializeAudioMaps(msg.id);
      }
    }
    } catch (e) {
      Get.snackbar('Error', 'Failed to load chat history');
  ...
  void initializeAudioMaps(String messageId) {
  playingMap.putIfAbsent(messageId, () => false.obs);
  pausedMap.putIfAbsent(messageId, () => false.obs);
  positionMap.putIfAbsent(messageId, () => Duration.zero.obs);
  durationMap.putIfAbsent(messageId, () => Duration.zero.obs);
}
  ...
  Future<void> playAudio(String path, String messageId) async {  
    ...
    _currentlyPlayingId = messageId;
    // CORRECTED: Initialize reactive variables for the messageId
    playingMap.putIfAbsent(messageId, () => true.obs);
    pausedMap.putIfAbsent(messageId, () => false.obs);
    positionMap.putIfAbsent(messageId, () => Duration.zero.obs);
    durationMap.putIfAbsent(messageId, () => Duration.zero.obs);
    ...
    try {
      // CORRECTED: Resolve path to a local file
      File? audioFile = await getAudioFileFromInput(path);
      if (audioFile == null) {
      ...
      await _player.startPlayer(
        ...
        whenFinished: () {
          playingMap[messageId]?.value = false;
          pausedMap[messageId]?.value = false;
          positionMap[messageId]?.value = Duration.zero;
          _currentlyPlayingId = null;
          update(); // CORRECTED: Notify UI of changes
        },
      );
  ...
  // Remove update(); Obx listens to .value changes.
      ...
      playingMap[messageId]?.value = true;
      pausedMap[messageId]?.value = false;
      update(); // CORRECTED: Notify UI of playback start
    } catch (e) {
  ...
  Future<void> pauseAudio(String messageId) async {
    ...
    playingMap[messageId]?.value = false;
    pausedMap[messageId]?.value = true;
    update(); // CORRECTED: Notify UI of pause
  }
  ...
  Future<void> resumeAudio(String messageId) async {
    ...
    playingMap[messageId]?.value = true;
    pausedMap[messageId]?.value = false;
    update(); // CORRECTED: Notify UI of resume
  }
  ...
  Future<void> stopAudio() async {
    ...
    playingMap.forEach((key, val) => val.value = false);
    pausedMap.forEach((key, val) => val.value = false);
    positionMap.forEach((key, val) => val.value = Duration.zero);
    update(); // CORRECTED: Notify UI of stop
  }
  ...
  Future<void> seekAudio(String messageId, Duration position) async {
    try {
      ...
      await _player.seekToPlayer(clampedPosition);
      positionMap[messageId]?.value = clampedPosition;
      update(); // CORRECTED: Notify UI of seek
    } catch (e) {
      Get.snackbar('Error', 'Failed to seek audio: $e');
    }
  }
